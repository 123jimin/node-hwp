		...
		case 'PARA_HEADER':
			...
			*/
			...
			if(range_i < rangeArr.length){
				...
				console.log(rangeArr.slice(range_i));
				console.log(record.toString());
				console.log(node.toHML(true));
				console.log(control_offset);
			}
			break;
		case 'PAGE_DEF':
		...
		case 'SHAPE_COMPONENT':
			...
			switch(record.attr._Type){
				case '$rec': // RECTANGLE
					...
					for(i=0;i<record.children.length;i++){
						switch(record.children[i].name){
							case 'SHAPE_COMPONENT_RECTANGLE':
								node.setAttr(record.children[i].attr);
								break;
						}
					}
					break;
