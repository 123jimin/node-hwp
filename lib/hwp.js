	...
	switch(record.name){
		...
		case 'SHAPE_COMPONENT':
			...
			switch(record.attr._Type){
				case '$lin': // LINE
					node = new HNode.LINE();
					drawingObject = node.getChild('DRAWINGOBJECT');
					node.setAttr(record.children[0].attr);
					break;
				case '$rec': // RECTANGLE
				...
				case '$pol': // POLYGON
					...
					drawingObject = node.getChild('DRAWINGOBJECT');
					var r_poly = record.children[0], point;
					for(i=0; i<r_poly.attr._Count; i++){
						point = new HNode.POINT();
						point.setAttr(r_poly.attr._Points[i]);
						node.add(point);
					}
					break;
				case '$arc': // ARC
				...
				case '$cur': // CURVE
					...
					drawingObject = node.getChild('DRAWINGOBJECT');
					var segment, r_c_attr = record.children[0].attr;
					for(i=1; i<r_c_attr._Count; i++){
						segment = new HNode.SEGMENT();
						segment.attr.X1 = r_c_attr._Points[i-1]._X;
						segment.attr.Y1 = r_c_attr._Points[i-1]._Y;
						segment.attr.X2 = r_c_attr._Points[i]._X;
						segment.attr.Y2 = r_c_attr._Points[i]._Y;
						segment.attr.Type = r_c_attr._Types[i-1].value;
						node.add(segment);
					}
					break;
				default:
			...
			if(drawingObject){
				...
				for(i=0; i<record.attr.RenderingInfo._Count; i++){
					var scaMatrix = new HNode.SCAMATRIX(), rotMatrix = new HNode.ROTMATRIX();
					scaMatrix.setAttr(record.attr.RenderingInfo._ScaRotMatrices[i].ScaMatrix);
					rotMatrix.setAttr(record.attr.RenderingInfo._ScaRotMatrices[i].RotMatrix);
				...
				if(record.attr.FillBrush){
					...
					// 비어있을 수 있음
					if(fillBrush.children.length > 0) toAdd.unshift(fillBrush);
				}
				...
				// TODO: LINESHAPE, FILLBRUSH, SHADOW 추가
			}
			break;
		default:
