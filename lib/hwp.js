...
HWP.prototype._getHML = function(parent, record){
	...
	switch(record.name){
		...
		case 'PAGE_DEF':
			...
			node.setAttrWithFilter(record.attr, function(name){return name != 'PageMargin'});
		...
		case 'PAGE_BORDER_FILL':
			...
			node.setAttrWithFilter(record.attr, function(name){
				return name != 'PageOffset';
			});
		...
		case 'CTRL_HEADER':
			...
			if(record.attr.ShapeObject){
				...
				shapeObject.setAttrWithFilter(record.attr.ShapeObject, function(name){
					if(name == 'TextWrap') return record.attr.ShapeObject.Position.TreatAsChar == false;
					return typeof record.attr.ShapeObject[name] != 'object' && name[0] != '_';
				});
			...
			switch(record.type){
				case 'secd': // SECDEF
					...
					node.setAttrWithFilter(record.attr, function(name){
						return name != 'StartNumber' && name != 'Hide';
					});
				...
				case 'cold': // COLDEF
					...
					node.setAttrWithFilter(record.attr, function(name){
						return typeof record.attr[name] != 'object' && name[0] != '_';
					});
					break;
				case 'tbl ': // TABLE
					...
					node.setAttrWithFilter(r_table.attr, function(name){
						return name != 'InsideMargin' && name != 'RowSizes';
					});
					node.getChild('INSIDEMARGIN').setAttr(r_table.attr.InsideMargin);
					var i, row_len, j, k, ind=1, r_list, r_cell, r_para, row, cell;
					for(i=0; i<r_table.attr.RowSizes.length; i++){
						row = new HNode.ROW();
						node.add(row);
						row_len = r_table.attr.RowSizes[i].value;
						for(j=0; j<row_len; j++){
							...
							cell = new HNode.CELL();
							cell.setAttrWithFilter(r_cell.attr, function(name){
								return name!='CellMargin' && name[0]!='_';
							});
							if(r_cell.attr.HasMargin){
								cell.getChild('CELLMARGIN').setAttr(r_cell.attr.CellMargin);
							}
							row.add(cell);
							for(k=0;k<r_list.attr.ParaCount;k++){
								...
								cell.add(this._getHML(cell, r_para));
							}
						}
					}
					break;
				case 'eqed': // EQUATION
					...
					node.setAttrWithFilter(record.children[0].attr, function(name){
						return name[0] != '_' && name != 'Script';
					});
