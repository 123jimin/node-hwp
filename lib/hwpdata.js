...
root.record.CHAR_SHAPE = function Record_CHAR_SHAPE(data){
	...
	this.attr.ShadowType=((tmp[1]&0x18)>>3);if(root.enum.ShadowType[this.attr.ShadowType]!==undefined)this.attr.ShadowType=root.enum.ShadowType[this.attr.ShadowType];
	...
	this.attr.ShadowOffsetX=this.data.readInt8(offset.value); offset.value+=1;
	this.attr.ShadowOffsetY=this.data.readInt8(offset.value); offset.value+=1;
	...
	this.attr.ShadowColor=this.data.readUInt32LE(offset.value); offset.value+=4;
	...
	// 그림자
	if(this.attr.ShadowType){
	this.attr.Shadow = {};
	
	this.attr.Shadow.Type = this.attr.ShadowType;
		this.attr.Shadow.OffsetX = this.attr.ShadowOffsetX;
		this.attr.Shadow.OffsetY = this.attr.ShadowOffsetY;
	}
	// TODO: 밑줄
	// TODO: 취소선
};
