...
root.record.BORDER_FILL = function Record_BORDER_FILL(data){
	...
	for(tmp=0;tmp<4;tmp++){
		...
		this.attr.BorderTypes[tmp].value=this.data.readUInt8(offset.value);this.attr.BorderTypes[tmp].value=root..enum.LineType1[this.attr.BorderTypes[tmp].value]; offset.value+=1;
	}
	this.attr.BorderWidths = [];
	for(tmp=0;tmp<4;tmp++){
		this.attr.BorderWidths[tmp] = {};
		this.attr.BorderWidths[tmp].value=this.data.readUInt8(offset.value);this.attr.BorderWidths[tmp].value=root..enum.LineWidth[this.attr.BorderWidths[tmp].value]; offset.value+=1;
	}
...
root.record.CHAR_SHAPE = function Record_CHAR_SHAPE(data){
	...
	this.attr.UnderlineType=((tmp[0]&0xc)>>2);
};
// 4.1.7 탭 정의
